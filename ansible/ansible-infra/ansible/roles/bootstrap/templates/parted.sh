...
check_dirty_partitions() {
    ...
    for i in $(seq 1 ${#exist_parts[@]}); do
        ...
        disk_name_index=$(( $(echo "$bdev" | awk '{print match($0,"[0-9]")}') - 1 ))
        partition_disk_name=${bdev:0:$disk_name_index}

	if [[ $disk_name != $partition_disk_name ]];then
	    echo "partition name is error! please check partition name for $bdev, disk $disk_name"
	    return 1
        fi
    done
    ...
    for i in $(seq 1 ${#exist_parts[@]}); do
        ...
        if [[ $(( last_end )) -lt $(( start )) ]];then
            echo "partition size not match(previous end ${last_end}MB is bigger than next start ${start}MB)! please check partitions for $disk_name"
            return 1
        fi
